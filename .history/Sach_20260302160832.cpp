...
void themCuonSach(DauSach* dauSach) {
    ...
    cout << "Nhap ma sach: ";
    cin >> newSach->maSach; // nhập mã sách cho cuốn sách mới thông qua con trỏ newSach. 
    // có nghĩa là khi người dùng nhập mã sách, giá trị đó sẽ được lưu vào thành phần maSach của cấu trúc Sach mà newSach đang trỏ tới.
    ...
    if (dauSach->dsSach.pHead == nullptr) {
        ...
        Sach* tail = dauSach->dsSach.pTail; // lấy con trỏ tail để trỏ đến cuốn sách cuối cùng hiện tại trong danh sách sách của đầu sách.
        tail->pNext = newSach; // cập nhật con trỏ pNext của cuốn sách cuối cùng hiện tại (tail) để trỏ đến newSach, vì newSach sẽ trở thành cuốn sách cuối cùng mới trong danh sách sau khi được thêm vào.
...
Sach* timSachTheoMa(const ListDauSach& ds, const char* maSach, DauSach*& dauSachOut)
{
    for (int i = 0; i < ds.n; i++)
    {
        ...
        while (temp != nullptr)
        {
            ...
            {
                dauSachOut = ds.nodes[i]; // gán đầu sách chứa cuốn sách tìm được vào con trỏ dauSachOut để trả về thông tin đầu sách đó cùng với cuốn sách.
                return temp;
            }
...
bool muonSach(TREE_DocGia root, ListDauSach &ds, int maThe, const char* maSach, Ngay ngayMuon) {
    ...
    if (docGia->dsMuonTra.pHead == nullptr) { // ds rỗng, thêm muonTra vào đầu danh sách mượn trả của độc giả.
        docGia->dsMuonTra.pHead = muonTra;
        docGia->dsMuonTra.pTail = muonTra;
    } else {
        docGia->dsMuonTra.pTail->pNext = muonTra;
        docGia->dsMuonTra.pTail = muonTra;
    }
}
...
bool traSach(TREE_DocGia root, ListDauSach &ds, int maThe, const char* maSach, Ngay ngayTra) {
    ...
    while (muonTra != nullptr) {
        if (strcmp(muonTra->maSach, maSach) == 0 && muonTra->trangThai == 0) {
            muonTra->trangThai = 1; // Đánh dấu trạng thái trả sách là 1 (đã trả)
            muonTra->ngayTra = ngayTra;

            // Tim Sach trong he thong 
            DauSach* dauSach = nullptr;
            Sach* sachTra = timSachTheoMa(ds, maSach, dauSach);
            if (sachTra != nullptr) {
                sachTra->trangThai = 0; // Cập nhật trạng thái của cuốn sách thành 0 (cho mượn được) sau khi trả.
    ...
    cout << "Khong tim thay cuon sach co ma: " << maSach << " trong danh sach muon cua doc gia co ma the: " << maThe << "\n";
    return false;
}
