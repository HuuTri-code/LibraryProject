...
bool muonSach(TREE_DocGia root, ListDauSach &ds, int maThe, const char* maSach, Ngay ngayMuon) {
    ...
    strcpy(muonTra->maSach, maSach); // Sao chép để đảm bảo rằng thông tin mã sách được lưu trữ chính xác trong cấu trúc MuonTra, giúp chúng ta dễ dàng truy cập và sử dụng thông tin này sau này khi cần thiết, chẳng hạn như khi in thông tin mượn sách hoặc khi xử lý trả sách.
}
