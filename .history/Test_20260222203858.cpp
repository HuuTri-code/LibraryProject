...
void redrawLine(const char* buffer, int len, int vt)
{
    ...
    if (moveLeft > 0)
        cout << "\033[" << moveLeft << "D"; // di chuyển con trỏ sang trái moveLeft vị trí, D lùi con trỏ về
...
void nhapMa(char* result, int maxLen)           
{
    ...
    while (true)
    {
        ...
        if ((c == 8 || c == 127)) // Backspace (8 trên Windows, 127 trên Unix)
        {
            if (vt > 0)
            {
