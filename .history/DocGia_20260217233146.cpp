...
// Số lượng mã thẻ tối đa từ 10000 đến 99999 là 90,000 mã
#define TOTAL_IDS 90000
#define START_ID 10000

struct MaThePool {
    int danhSachMa[TOTAL_IDS];
    int indexHienTai;
};

// Hàm hoán vị thuần túy
...
// 1. Khởi tạo kho mã thẻ - Chạy 1 lần duy nhất khi mở chương trình
void khoiTaoKhoMa(MaThePool &pool) {
    // Nạp mã thẻ theo thứ tự tăng dần
    for (int i = 0; i < TOTAL_IDS; i++) {
        pool.danhSachMa[i] = START_ID + i;
    }

    // Fisher-Yates Shuffle: Thuật toán trộn mảng tối ưu O(n)
    srand(time(0)); // Reset hạt giống ngẫu nhiên theo thời gian thực
    for (int i = TOTAL_IDS - 1; i > 0; i--) {
        // Chọn một vị trí ngẫu nhiên từ 0 đến i
        int j = rand() % (i + 1);
        hoanVi(pool.danhSachMa[i], pool.danhSachMa[j]);
    }
    
    pool.indexHienTai = 0; // Bắt đầu lấy từ vị trí đầu tiên
}

// 2. Lấy mã mới - Tốc độ cực nhanh O(1)
int layMaTheMoi(MaThePool &pool) {
    if (pool.indexHienTai < TOTAL_IDS) {
        return pool.danhSachMa[pool.indexHienTai++];
    }
    return -1; // Đã hết mã thẻ để cấp
}