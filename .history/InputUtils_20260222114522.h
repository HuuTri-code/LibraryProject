...
using namespace std;
    ...
    inline void setupConsole() { // thiết lập chế độ "nhạy" cho terminal, cho phép đọc phím mà không cần nhấn Enter và tắt hiển thị ký tự khi nhập.
        ...
        struct termios new_term = old_term; // tạo một bản sao của cấu hình cũ để chỉnh sửa mà không ảnh hưởng đến cấu hình gốc.
        new_term.c_lflag &= ~(ICANON | ECHO);
        tcsetattr(STDIN_FILENO, TCSANOW, &new_term); 
        // tcsetattr là một hàm trong thư viện termios.h của hệ điều hành Unix/Linux, được sử dụng để thiết lập các thuộc tính của terminal.
    }
    ...
    inline int getch_raw() { // đọc một ký tự từ bàn phím mà không cần nhấn Enter, trả về mã ASCII của ký tự đó. Nếu có lỗi trong quá trình đọc, hàm sẽ trả về 0.
        ...
        if (read(STDIN_FILENO, &ch, 1) <= 0) return 0;
           return (unsigned char)ch; // Nên ép kiểu unsigned để tránh giá trị âm
    }
