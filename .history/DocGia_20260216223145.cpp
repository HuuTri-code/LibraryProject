...
DocGia* timDocGia(const TREE_DocGia root, int maThe) {
    /*/ Quy tắc là: Đi hết bên trái, rồi mới in gốc, rồi mới đi bên phải.
        * Nếu maThe nhỏ hơn maThe của gốc, thì tiếp tục tìm kiếm trong cây con bên trái.
        * Nếu maThe lớn hơn maThe của gốc, thì tiếp tục tìm kiếm trong cây con bên phải.
        * Nếu maThe bằng maThe của gốc, thì đã tìm thấy độc giả cần tìm và trả về địa chỉ của nút hiện tại.
        * Nếu cây rỗng hoặc đã duyệt hết cây mà không tìm thấy độc giả có mã thẻ trùng khớp, thì trả về nullptr để báo hiệu rằng không tìm thấy độc giả cần tìm.
        */
    ...
    while (current != nullptr) {
        if (maThe < current->maThe) {
            ...
            return current; // trả về địa chỉ của nút hiện tại nếu mã thẻ trùng khớp.
        }
    }
}