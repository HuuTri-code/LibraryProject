...
void muonSach(TREE_DocGia root, ListDauSach &ds, int maThe, const char* maSach, Ngay ngayMuon) {
    ...
    if (sach->trangThai != 0) {
        cout << "Cuon sach nay khong cho muon duoc!\n";
        return;
    }

    // Cập nhật trạng thái sách và thêm vào danh sách mượn của độc giả
    sach->trangThai = 1; // Đánh dấu sách đã được mượn

    MuonTra* newMuonTra = new MuonTra;
    strcpy(newMuonTra->maSach, maSach);
    newMuonTra->ngayMuon = ngayMuon;
    newMuonTra->trangThai = 0; // Đang mượn
    newMuonTra->pNext = nullptr;

    if (docGia->dsMuonTra.pHead == nullptr) {
        docGia->dsMuonTra.pHead = newMuonTra;
        docGia->dsMuonTra.pTail = newMuonTra;
    } else {
        docGia->dsMuonTra.pTail->pNext = newMuonTra;
        docGia->dsMuonTra.pTail = newMuonTra;
    }
