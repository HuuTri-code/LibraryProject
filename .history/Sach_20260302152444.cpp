...
bool muonSach(TREE_DocGia root, ListDauSach &ds, int maThe, const char* maSach, Ngay ngayMuon) {
    ...
    MuonTra* muonTra = new MuonTra; // lý do tạo con trỏ muonTra là để cấp phát bộ nhớ cho một đối tượng MuonTra mới, đại diện cho một lần mượn sách của độc giả. Con trỏ muonTra sẽ được sử dụng để lưu trữ thông tin về cuốn sách được mượn, ngày mượn, ngày trả và trạng thái của lần mượn đó. Việc sử dụng con trỏ giúp chúng ta có thể dễ dàng quản lý và cập nhật thông tin về các lần mượn sách của độc giả trong danh sách mượn trả của họ.
}
