...
using namespace std;
    ...
    inline void setupConsole() { // thiết lập chế độ "nhạy" cho terminal, cho phép đọc phím mà không cần nhấn Enter và tắt hiển thị ký tự khi nhập.
        ...
        // 2 dòng dưới để đảm bảo rằng read() sẽ trả về ngay khi có một ký tự được nhập, mà không cần phải đợi Enter, và không có timeout.
    ...
    inline int getch_raw() { // đọc một ký tự từ bàn phím mà không cần nhấn Enter, trả về mã ASCII của ký tự đó. Nếu có lỗi trong quá trình đọc, hàm sẽ trả về 0.
        ...
        //return (int)ch; // dùng kiểu int để có thể trả về mã phím đặc biệt lớn hơn 255.
            ...
            unsigned char ch;
    // Ép STDIN phải trả kết quả ngay lập tức, không đợi buffer của Terminal
