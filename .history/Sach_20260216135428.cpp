...
bool xoaDauSach(ListDauSach &ds, const char* ISBN) {
    for (int i = 0; i < ds.n; i++) {
        // so sánh mã ISBN của đầu sách tại vị trí i với mã ISBN cần xóa. Nếu tìm thấy đầu sách có mã ISBN trùng khớp, tiến hành kiểm tra trạng thái của tất cả cuốn sách thuộc đầu sách đó trước khi xóa.
        if (strcmp(ds.nodes[i]->ISBN, ISBN) == 0) {
            // kiểm tra trạng thái của tất cả cuốn sách thuộc đầu sách này trước khi xóa. Nếu có bất kỳ cuốn sách nào đang được mượn (trangThai == 1), không thể xóa đầu sách.
            ...
            // nếu tất cả cuốn sách thuộc đầu sách này đều không đang được mượn, tiến hành xóa đầu sách khỏi danh sách đầu sách.
            delete ds.nodes[i]; // giải phóng bộ nhớ đã cấp phát cho đầu sách tại
            ds.n--; // giảm đầu sách.  

        } else {
            cout << "Khong tim thay dau sach co ISBN: " << ISBN << endl;
            return false;
        }
    }
}
