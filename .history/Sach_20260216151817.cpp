...
// Độ phức tạp của hàm là O(n) trường hợp xấu nhất khi cuốn sách cần xóa là cuốn sách cuối cùng trong danh sách hoặc không tồn tại trong danh sách, vì phải duyệt qua toàn bộ danh sách để tìm kiếm cuốn sách đó. Trường hợp tốt nhất là O(1) khi cuốn sách cần xóa là cuốn sách đầu tiên trong danh sách, vì chỉ cần kiểm tra một lần và cập nhật con trỏ pHead. Trường hợp trung bình là O(n/2) khi cuốn sách cần xóa nằm ở giữa danh sách, nhưng vẫn được coi là O(n) do tính chất của phép so sánh Big O.
...
void inDanhSachCuonSach(const DauSach* dauSach) {
    ...
    cout << "Danh sach cuon sach cua dau sach: " << dauSach->tenSach << endl;
    ...
    while (temp != nullptr) {
        ...
        if (temp->trangThai == 0) {
            cout << "Cho muon duoc";
        } else if (temp->trangThai == 1) {
            cout << "Da muon";
        } else if (temp->trangThai == 2) {
            cout << "Thanh ly";
        }
        cout << endl;
        temp = temp->pNext;
