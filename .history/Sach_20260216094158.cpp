...
bool xoaCuonSach(DauSach* dauSach, const char* maSach) {
    ...
    while (current != nullptr) {
        ...
        if (strcmp(current->maSach, maSach) == 0) {
            ...
            delete current; // giải phóng bộ nhớ của cuốn sách đã xóa.
            dauSach->dsSach.tongSoSach--; // giảm tổng số sách của đầu sách đi 1.
            return true; // trả về true nếu xóa thành công.
        }
        prev = current;
        current = current->pNext;
    }
    return false; // trả về false nếu không tìm thấy cuốn sách có mã sách cần xóa.
}