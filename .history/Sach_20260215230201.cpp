...
void themCuonSach(DauSach* dauSach) {
    ...
    if (dauSach->dsSach.pHead == nullptr) {
        ...
        dauSach->dsSach.pTail->pNext = newSach; // nếu danh sách sách của đầu sách hiện tại không rỗng, thì con trỏ pNext của cuốn sách cuối cùng hiện tại sẽ trỏ đến newSach, sau đó cập nhật con trỏ pTail của đầu sách hiện tại để trỏ đến newSach, vì newSach bây giờ là cuốn sách cuối cùng trong danh sách.
