        ...
        if (isalnum(c) || c == '-' || c == '_') 
        {
            if (len < maxLen - 1) 
            {
                    if (isalpha((unsigned char)c)) 
                        c = toupper(c); // nếu c là một ký tự chữ cái, chuyển nó thành chữ hoa bằng cách sử dụng hàm toupper. Hàm này sẽ trả về phiên bản chữ hoa của ký tự nếu nó là một chữ cái thường, hoặc trả về chính nó nếu nó không phải là một chữ cái thường.
                    result[len++] = (char)c;
                    cout << (char)c << flush;
            }  else 
                {
                    cout << "\a" << flush; // Đầy chỗ.
                 }
        }
    }   
