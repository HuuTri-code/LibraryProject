...
void nhapMa(char* result, int maxLen)
{
    ...
    while (true)
    {
        ...
        {
            if (len > 0) {
                result[len] = '\0'; // kết thúc chuỗi bằng ký tự null để đảm bảo rằng biến result chứa một chuỗi hợp lệ.
                cout << endl; // in một dòng mới sau khi người dùng nhấn Enter để kết thúc việc nhập mã.
                return; // thoát khỏi hàm sau khi hoàn thành việc nhập mã.
            } else {
                continue; // nếu người dùng nhấn Enter mà chưa nhập gì, tiếp tục vòng lặp để cho phép họ nhập lại.
            }
