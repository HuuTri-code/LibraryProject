...
struct Sach {
    ...
    // (Nếu em load dữ liệu từ file thì sao? - khi DocGia* dg = new DocGia;, nó tự động trangThaiThe = THE_HOAT_DONG; --> Nhưng sau đó bạn lại đọc file và gán lại.→ Việc gán mặc định này thực ra vô nghĩa.)
    char viTri[20];
...
struct ListSach {
    ...
    int tongSoSach = 0; // đếm tổng số sách để thống kê, không cần phải duyệt lại danh sách để đếm nữa, chỉ cần tăng khi thêm sách và giảm khi xóa sách.
};
