...
bool xoaDauSach(ListDauSach &ds, const char* ISBN) {
    for (int i = 0; i < ds.n; i++) {
        if (strcmp(ds.nodes[i]->ISBN, ISBN) == 0) {
            ...
            // toDelete sau mỗi lần duyệt sẽ được cập nhật để trỏ đến đầu sách tiếp theo trong danh sách cho đến khi tìm thấy đầu sách có ISBN trùng khớp với ISBN cần xóa. Khi tìm thấy, toDelete sẽ trỏ đến đầu sách đó và chúng ta sẽ thực hiện các bước tiếp theo để xóa đầu sách đó khỏi danh sách.
