...
// Hàm thiết lập chế độ console để nhận phím mà không cần nhấn Enter
#ifdef _WIN32
    #include <conio.h>
    ...
    inline int getKey() {
        ...
        if (ch == 27) { // Bắt đầu chuỗi Escape (phím mũi tên trên Linux)
            ...
            if (read(STDIN_FILENO, &seq[0], 1) > 0 && read(STDIN_FILENO, &seq[1], 1) > 0) { // Đọc tiếp 2 ký tự sau khi đọc được ESC, nếu không đọc được thì trả về KEY_ESC
                if (seq[0] == '[') {
                    switch (seq[1]) {
