...
using namespace std;
    ...
    inline void setupConsole() { // thiết lập chế độ "nhạy" cho terminal, cho phép đọc phím mà không cần nhấn Enter và tắt hiển thị ký tự khi nhập.
        ...
        // Tắt chế độ Canonical (ICANON) để đọc từng ký tự ngay lập tức mà không cần nhấn Enter, tắt ECHO để không hiển thị ký tự khi nhập, tắt ISIG để vô hiệu hóa tín hiệu như Ctrl+C, và tắt IEXTEN để vô hiệu hóa các chức năng mở rộng của terminal.
        new_term.c_lflag &= ~(ICANON | ECHO);

        // Tắt các chế độ xử lý đặc biệt của đầu vào như BRKINT (ngắt khi có tín hiệu ngắt), ICRNL (chuyển đổi carriage return thành newline), INPCK (kiểm tra lỗi parity), ISTRIP (loại bỏ bit thứ 8), và IXON (tắt kiểm soát luồng phần mềm).
        new_term.c_iflag &= ~(BRKINT | ICRNL | INPCK | ISTRIP | IXON);

        // Tắt các chế độ xử lý đặc biệt của đầu ra như OPOST (xử lý đặc biệt của đầu ra) để đảm bảo rằng các ký tự được in ra đúng như chúng được gửi đến terminal mà không bị thay đổi hoặc xử lý thêm.
        new_term.c_oflag &= ~(OPOST);

        // Đảm bảo đọc từng ký tự
        new_term.c_cc[VMIN] = 1;  // 1 ký tự là đủ để read() trả về.
        new_term.c_cc[VTIME] = 0;  // 0 deci-giây, tức là không có timeout.
