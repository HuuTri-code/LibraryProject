...
bool muonSach(TREE_DocGia root, ListDauSach &ds, int maThe, const char* maSach, Ngay ngayMuon) {
    ...
    MuonTra* muonTra = new MuonTra; // tạo một con trỏ muonTra và cấp phát bộ nhớ cho một đối tượng MuonTra mới. Đây sẽ là thông tin về việc mượn sách mà chúng ta muốn thêm vào danh sách mượn trả của độc giả hiện tại. Sau khi cấp phát bộ nhớ, muonTra sẽ trỏ đến vùng nhớ chứa thông tin của việc mượn sách mới mà chúng ta sẽ nhập liệu từ người dùng.
}
