...
void DDEULER(int A[][MAX], int n, int u) {
    ...
    dCE = 0;
    do {
        v = stack[top]; // Lay dinh o dinh Stack de xet, v = dinh dang xet hien tai, ban dau la dinh u
        x = 1; // Khoi tao x de tim canh noi v voi x, cho bang 1 de bat dau tim tu dinh 1

        while (x <= n && A[v][x] == 0) x++; // Tim canh noi v voi x, neu khong co canh nao noi v voi x thi tang x len 1 de tim tiep
        
        if (x > n) { // Neu dinh v khong con canh noi nao nua
            dCE++;  // Tang bien dem dCE len 1 de luu vi tri cua dinh v trong mang CE
