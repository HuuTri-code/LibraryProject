...
{
    cout << "\r" << "Test nhap ma: "; 
    if (len > 0) cout.write(buffer, len);
   //  cout << buffer; sẽ in mảng cho đến khi nó gặp ký tự kết thúc chuỗi (\0). Còn cout.write(buffer, len); sẽ in đúng số ký tự được chỉ định bởi len, bất kể có ký tự \0 ở đâu trong mảng hay không.
