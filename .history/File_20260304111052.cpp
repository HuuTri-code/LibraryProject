...
bool loadDauSach(const char* filename, ListDauSach& ds)
{
    ...
    while (getline(file, line))  // Đọc từng dòng trong file
    {
        ...
        stringstream ss(line); // Tạo stringstream để phân tích dòng
        string ISBN, tenSach, tacGia, theLoai;  // Biến để lưu thông tin đầu sách
        int soTrang, namXB, soLuotMuon, tongSoSach; // Biến để lưu thông tin số trang, năm xuất bản, số lượt mượn và tổng số sách

        getline(ss, ISBN, '|'); // Đọc ISBN, phân tách bằng '|'
        getline(ss, tenSach, '|');
        getline(ss, tacGia, '|');
        getline(ss, theLoai, '|');

        ss >> soTrang; // Đọc số trang
        ss.ignore(1); // Bỏ qua ký tự phân tách
        ss >> namXB;
        ss.ignore(1);
        ss >> soLuotMuon;
        ss.ignore(1);
        ...
        strcpy(dsNew->ISBN, ISBN.c_str());  // Sao chép chuỗi từ string sang char array, ISBN.c_str là con trỏ đến dữ liệu của string, có nghĩa là lấy chuỗi C-style từ string
...
void capNhatTrangThaiSachDangMuon(TREE_DocGia root, ListDauSach& ds)
{
    ...
    MuonTra* mt = root->docGia.dsMuonTra.pHead;
    while (mt)
    {
        if (mt->trangThai == 0)
        {
