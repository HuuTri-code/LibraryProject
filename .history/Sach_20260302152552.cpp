...
bool muonSach(TREE_DocGia root, ListDauSach &ds, int maThe, const char* maSach, Ngay ngayMuon) {
    ...
    strcpy(muonTra->maSach, maSach); // sao chép mã sách vào thành phần maSach của cấu trúc MuonTra mà muonTra đang trỏ tới, để lưu thông tin về cuốn sách mà độc giả muốn mượn.
}
