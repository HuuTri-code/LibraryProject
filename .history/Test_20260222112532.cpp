...
void nhapMa(char* result, int maxLen)
{
    ...
    while (true)
    {
        ...
        if (isalnum(c) || c == '-' || c == '_') 
        {
            if (len < maxLen - 1) 
            {
                    if (isalpha((unsigned char)c)) c = toupper(c); // dùng unsigned char để đảm bảo rằng hàm isalpha hoạt động chính xác với các ký tự có giá trị ASCII mở rộng (trên 127) và tránh lỗi khi xử lý các ký tự không phải là chữ cái.
                    result[len++] = (char)c;
                    cout << (char)c << flush;
            }  else 
                {
