...
DocGia* timDocGia(const TREE_DocGia root, int maThe) {
    ...
    if (maThe == root->maThe) {
        reutrn root; // trả về con trỏ đến nút hiện tại nếu mã thẻ của nút hiện tại trùng với mã thẻ cần tìm, cho phép người gọi có thể truy cập thông tin của độc giả này.
    }
}