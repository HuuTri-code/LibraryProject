...
void inDanhSachCuonSach(const DauSach* dauSach) {
    ...
    Sach* temp = dauSach->dsSach.pHead; // tác dụng là để lấy con trỏ tạm temp và gán nó bằng con trỏ pHead của danh sách sách của đầu sách được truyền vào hàm. Điều này cho phép chúng ta sử dụng con trỏ temp để duyệt qua danh sách các cuốn sách của đầu sách đó mà không làm thay đổi con trỏ pHead gốc, giúp chúng ta dễ dàng in thông tin của từng cuốn sách trong danh sách mà không ảnh hưởng đến cấu trúc dữ liệu ban đầu của đầu sách.
