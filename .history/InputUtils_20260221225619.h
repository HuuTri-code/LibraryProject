...
using namespace std;
    ...
    inline void setupConsole() { // thiết lập chế độ "nhạy" cho terminal, cho phép đọc phím mà không cần nhấn Enter và tắt hiển thị ký tự khi nhập.
        ...
        // giải thích: tcgetattr(STDIN_FILENO, &old_term) lấy cấu hình hiện tại của terminal và lưu vào biến old_term để có thể khôi phục sau này. Sau đó, một bản sao của cấu hình cũ được tạo ra trong biến new_term, và các cờ ICANON và ECHO được tắt để cho phép đọc phím mà không cần nhấn Enter và tắt hiển thị ký tự khi nhập. Cuối cùng, tcsetattr(STDIN_FILENO, TCSANOW, &new_term) áp dụng cấu hình mới cho terminal ngay lập tức.
    }
