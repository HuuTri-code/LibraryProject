...
Sach* timSachTheoMa(const ListDauSach& ds, const char* maSach, DauSach*& dauSachOut)
{
    for (int i = 0; i < ds.n; i++)
    {
        ...
        while (temp != nullptr)
        {
            ...
            {
                dauSachOut = ds.nodes[i]; // mục đích chính là để trả về thông tin về đầu sách chứa cuốn sách có mã sách trùng khớp, giúp người dùng có thể biết được cuốn sách đó thuộc đầu sách nào trong danh sách đầu sách hiện tại. Bằng cách sử dụng con trỏ dauSachOut để lưu trữ địa chỉ của đầu sách chứa cuốn sách cần tìm, chúng ta có thể dễ dàng truy cập và hiển thị thông tin của đầu sách đó sau khi tìm thấy cuốn sách có mã sách trùng khớp, cung cấp cho người dùng một cái nhìn tổng quan về cuốn sách đó trong ngữ cảnh của đầu sách mà nó thuộc về.
                return temp;
            }
