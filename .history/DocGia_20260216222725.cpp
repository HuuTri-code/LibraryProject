...
DocGia* timDocGia(const TREE_DocGia root, int maThe) {
   ...
   /*/ if (maThe < root->maThe) {
    ...
    // viết thuần tuý bằng vòng lặp
    DocGia* current = root; // khởi tạo con trỏ current, an toàn hơn dùng root trực tiếp để tránh thay đổi root khi duyệt cây.
    while (current != nullptr) {
        if (maThe < curretn->maThe) {
            current = current->pLeft; // di chuyển con trỏ current đến cây con bên trái của nút hiện tại để tiếp tục tìm kiếm.
        } else if (maThe > current->maThe) {
            current = current->pRight; // di chuyển con trỏ current đến cây con bên phải của nút hiện tại để tiếp tục tìm kiếm.
        } else {
