...
using namespace std;
    ...
    inline int getch_raw() { // đọc một ký tự từ bàn phím mà không cần nhấn Enter, trả về mã ASCII của ký tự đó. Nếu có lỗi trong quá trình đọc, hàm sẽ trả về 0.
        ...
        return (int)ch; // unsigned là để đảm bảo rằng giá trị trả về luôn dương, tránh trường hợp khi đọc một ký tự có mã ASCII lớn hơn 127, nó sẽ được hiểu là một số âm nếu không được chuyển đổi sang unsigned char.
    }
