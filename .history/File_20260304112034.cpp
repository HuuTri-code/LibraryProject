...
{
    ifstream file(filename);    // làm việc với file DauSach.txt, nếu file không tồn tại hoặc không thể mở được, chương trình sẽ in ra thông báo lỗi và trả về false để báo hiệu rằng việc tải dữ liệu đã thất bại. Nếu file được mở thành công, chương trình sẽ tiếp tục đọc dữ liệu từ file để tạo danh sách đầu sách và các cuốn sách con của chúng, sau đó đóng file và trả về true để báo hiệu rằng việc tải dữ liệu đã thành công.
