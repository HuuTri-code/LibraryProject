...
int Kiemtra(int A[][MAX], int n, int *u) {
    ...
    for (i = 1; i <= n; i++) {
        ...
        if (s % 2 != 0) {
            d++;    // Dem so dinh bac le, vi du: dinh co bac 3, 5, 7... la dinh bac le
...
// Thuat toan tim duong di Euler bang Stack (Giong thuat toan Hierholzer)
void DDEULER(int A[][MAX], int n, int u) {
    ...
    do {
        v = stack[top]; 
        x = 1;
        // Tim dinh x dau tien co canh noi voi v
        while (x <= n && A[v][x] == 0) x++;
