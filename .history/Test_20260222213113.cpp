...
{
    cout << "\r" << "Test nhap ma: "; 
    if (len > 0) cout.write(buffer, len);   //  cout << buffer; sẽ in mảng cho đến khi nó gặp ký tự kết thúc chuỗi (\0). Còn cout.write(buffer, len); sẽ in đúng số ký tự được chỉ định bởi len, bất kể có ký tự \0 ở đâu trong mảng hay không.
    ...
    while (true)
    {
        ...
        if (c == 27) // do phím mũi tên trên Ubuntu gửi về 3 ký tự: ESC, '[', và một ký tự khác để xác định phím cụ thể (ví dụ: 'A' cho UP, 'B' cho DOWN, 'C' cho RIGHT, 'D' cho LEFT).
        {
            ...
            if (c1 == '[') // là mã bắt đầu của chuỗi phím đặc biệt (ví dụ: mũi tên)
            {
                ...
                else if (c2 == '3') // Delete trên Ubuntu gửi thêm một ký tự nữa sau '3', thường là '~' ví dụ Phím Delete: ESC [ 3 ~
                {
                    int c3 = getch_raw();
                    if (c3 == '~') // ~ là mã phím DELETE trên Ubuntu
                    {
                        if (vt < len)
                        {
...
void testNhapLieu()
{
    ...
    cout << "Dia chi bat dau mang: " << (void*)ma << "\r\n";// "\r\n"; in địa chỉ của mảng ma dưới dạng con trỏ void* để tránh việc cout cố gắng in mảng như một chuỗi ký tự (nếu ma chứa ký tự '\0' sẽ bị cắt ngắn khi in). Việc này giúp chúng ta thấy rõ địa chỉ bắt đầu của mảng ma trong bộ nhớ, điều này hữu ích để kiểm tra xem hàm nhapMa có ghi vượt quá giới hạn của mảng hay không
    cout << "Dia chi ket thuc mang: " << (void*)(ma + GIOI_HAN - 1) << "\r\n";

    cout << "Test nhap ma: " << flush; // đảm bảo rằng thông báo được in ra trước khi hàm nhapMa bắt đầu nhận input từ người dùng.
    nhapMa(ma, GIOI_HAN);
    cout << "=> Ma vua nhap: [" << ma << "]" << endl << flush;
}
