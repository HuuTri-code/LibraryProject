...
void inDanhSachDocGia(const TREE_DocGia root) {
    if (root == nullptr) return; // đệ quy tới khi gặp nút nullptr, tức là đã duyệt hết cây hoặc cây rỗng, thì dừng lại và trả về để không in gì cả.
    ...
    inDanhSachDocGia(root->pRight); // gọi đệ quy từ cây con bên phải để in danh sách độc giả trong cây con bên phải sau khi đã in xong thông tin của nút gốc.
}