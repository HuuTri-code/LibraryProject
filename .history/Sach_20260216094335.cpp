        ...
        if (strcmp(current->maSach, maSach) == 0) {
            if (current == dauSach->dsSach.pHead && current == dauSach->dsSach.pTail) {
                ...
                if (current == dauSach->dsSach.pHead) {
                    ...
                    // nếu cuốn sách cần xóa không phải là cuốn sách đầu tiên, cập nhật con trỏ pNext của cuốn sách trước đó (prev) để trỏ đến cuốn sách tiếp theo sau khi xóa cuốn sách hiện tại.
