...
bool xoaDauSach(ListDauSach &ds, const char* ISBN) {
    for (int i = 0; i < ds.n; i++) {
        if (strcmp(ds.nodes[i]->ISBN, ISBN) == 0) {
            ...
            while (temp != nullptr) {
                Sach* next = temp->pNext; // lưu con trỏ đến cuốn sách tiếp theo trước khi giải phóng bộ nhớ của cuốn sách hiện tại, để tránh mất truy cập đến phần còn lại của danh sách sau khi xóa một cuốn sách.
                delete temp;
                temp = next;
            }
