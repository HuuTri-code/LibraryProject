...
bool xoaCuonSach(DauSach* dauSach, const char* maSach) {
    ...
    while (current != nullptr) {
        ...
        if (strcmp(current->maSach, maSach) == 0) {
            ...
            // nếu cuốn sách cần xóa là cuốn sách duy nhất trong danh sách (cả pHead và pTail đều trỏ đến cuốn sách đó), cập nhật cả pHead và pTail thành nullptr để biểu thị rằng danh sách sách của đầu sách hiện tại đã trở nên rỗng sau khi xóa cuốn sách đó.
            if (current == dauSach->dsSach.pHead && current == dauSach->dsSach.pTail) {
                dauSach->dsSach.pHead = nullptr;
                dauSach->dsSach.pTail = nullptr;
                dauSach->dsSach.tongSoSach = 0;
            } else {
