...
using namespace std;
    ...
    inline void setupConsole()
    {
        ...
        raw = old_term; // bắt đầu với cấu hình cũ và chỉnh sửa nó để tạo ra cấu hình mới.

        raw.c_lflag &= ~(ICANON | ECHO); // tắt chế độ canonical (đọc dòng) và echo (hiển thị ký tự khi nhập).
        raw.c_iflag &= ~(IXON | ICRNL); // tắt chế độ kiểm soát dòng (Ctrl-S/Ctrl-Q) và chuyển đổi carriage return thành newline.
        raw.c_oflag &= ~(OPOST); // tắt chế độ xử lý đầu ra (output processing), để đảm bảo rằng các ký tự được in ra ngay lập tức mà không bị thay đổi.

        // Thiết lập VMIN và VTIME để đọc một ký tự ngay khi nó được nhập mà không cần đợi Enter.
        raw.c_cc[VMIN] = 1;
        raw.c_cc[VTIME] = 0;
