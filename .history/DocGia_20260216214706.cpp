...
bool themDocGia(TREE_DocGia& root, DocGia* newnode) {
    ...
    // nếu mã thẻ của độc giả mới (newnode->maThe) nhỏ hơn mã thẻ của nút gốc hiện tại (root->maThe),
    // thì tiếp tục đệ quy vào cây con bên trái (root->pLeft) để tìm vị trí thích hợp để thêm nút mới.
